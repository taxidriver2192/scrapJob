...
// LinkedIn skills and work type extraction
console.log('Loading skills extraction functions...');

const getWorkTypeAndSkills = function(): { workType: string; skills: string[] } {
    const result = { workType: '', skills: [] as string[] };
    ...
    for (const selector of modalSelectors) {
        modal = Utils.safeQuery<HTMLElement>(selector);
        if (modal) {
    ...
    if (modal) {
        ...
        for (const skillEl of skillElements) {
            ...
            if (skillTextEl && skillTextEl.textContent) {
                const skillName = skillTextEl.textContent.trim();
        ...
        for (const selector of descriptionSelectors) {
            const desc = Utils.safeQuery<HTMLElement>(selector);
        ...
        for (const selector of skillContainerSelectors) {
            const container = Utils.safeQuery<HTMLElement>(selector);
